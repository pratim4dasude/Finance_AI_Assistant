...
#             }
#
#         total_value = sum(self._holding_value(h) for h in portfolio)
#
#         positions = []
#         for h in portfolio:
#             value = self._holding_value(h)
#             pct = (value / total_value * 100) if total_value > 0 else 0
#             ticker = h.get("ticker") or h.get("symbol")
#
#             # positions.append(
...
#     #         return float(h.get("market_value") or 0)
#     #
#     #     quantity = float(h.get("quantity", 0) or 0)
#     #     price = float(
#     #         h.get("avg_price", h.get("avg_cost", h.get("price", 0))) or 0
#     #     )
#     #
...
#             h["latest_price"] = round(latest_price, 2)
#             h["price_source"] = "yfinance"
#             return quantity * latest_price
#
#         # Fallback to provided market value
#         if h.get("market_value") is not None:
#             h["price_source"] = "provided_market_value"
#             return float(h.get("market_value") or 0)
#
#         # Final fallback → avg_cost
#         price = float(
#             h.get("avg_price", h.get("avg_cost", h.get("price", 0))) or 0
#         )
#         h["price_source"] = "avg_cost_fallback"
#
#         return quantity * price
#
#     def _benchmark(self, currency: str | None):
...
#             }
#
#         positions = [self._enrich_holding(h) for h in portfolio]
#
...
#         }
#
#     def _enrich_holding(self, h: dict) -> dict:
#         ticker = h.get("ticker") or h.get("symbol")
#         quantity = float(h.get("quantity", 0) or 0)
#
#         latest_price = get_latest_price(ticker) if ticker else None
...
#         elif h.get("market_value") is not None and quantity > 0:
#             price_used = float(h.get("market_value") or 0) / quantity
...
#
#         avg_cost = h.get("avg_cost") or h.get("avg_price")
#         unrealized_return_pct = None
#
#         if avg_cost and latest_price:
#             avg_cost = float(avg_cost)
#             unrealized_return_pct = round(((latest_price - avg_cost) / avg_cost) * 100, 2)
#
#         return {
#             "ticker": ticker,
#             "name": h.get("name", ticker),
